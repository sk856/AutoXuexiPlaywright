...
# Relative imports
from .common import (
    cache, tasks_to_be_done, register_tasks, clean_tasks, reset_processing_pause,
)
...
def _on_processor_started():
    ...
    if _config.async_mode:
        from .async_api.test import DailyTestTask, FunTestTask, WeeklyTestTask, SpecialTestTask
        from .async_api.login import LoginTask
        from .async_api.read import VideoTask, NewsTask
    else:
        from .sync_api.test import DailyTestTask, FunTestTask, WeeklyTestTask, SpecialTestTask
        from .sync_api.login import LoginTask
        from .sync_api.read import VideoTask, NewsTask
    if not register_tasks(LoginTask, NewsTask, VideoTask, DailyTestTask, FunTestTask, WeeklyTestTask, SpecialTestTask):
