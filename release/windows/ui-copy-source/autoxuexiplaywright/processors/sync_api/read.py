...
class VideoTask(_ReadTask):
    ...
    def __enter__(self):
        ...
        if not self._wait_locator(text_wrappers.last, timeout=_READ_LIST_TIMEOUT_MSECS):
        ...
        if not self._wait_locator(text_wrappers.last, timeout=_READ_LIST_TIMEOUT_MSECS):
            if self.last_page.locator(ReadSelectors.VIDEO_PLAYER).count() > 0 or self.last_page.locator(ReadSelectors.PAGE_PARAGRAPHS).count() > 0:
