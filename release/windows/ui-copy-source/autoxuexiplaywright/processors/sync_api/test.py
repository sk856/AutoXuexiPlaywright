...
class _TestTask(Task):
    ...
    def finish(self) -> bool:
        ...
        while not self._is_test_finished():
            ...
            while (answer_items.count() <= 0 and blanks.count() <= 0
                   and monotonic() < answer_ready_deadline):
                self.last_page.wait_for_timeout(100)
            if answer_items.count() <= 0 and blanks.count() <= 0:
    ...
    def _get_answer_from_page(self) -> list[str]:
        ...
        try:
            ...
            if "ant-popover-open" not in (tips.get_attribute("class") or ""):
                tips.click(timeout=_ACTION_TIMEOUT_MSECS)
            ...
            if visible_popover is not None:
                ...
                if self._wait_locator(font.last, timeout=WAIT_CHOICE_SECS * 1000):
                    for answer in [clean_string(text) for text in font.all_inner_texts()]:
            ...
            try:
                if tips.count() > 0 and "ant-popover-open" in (tips.get_attribute("class") or ""):
                    tips.click(timeout=3000)
            except Exception:
                pass
    ...
    def _get_answer_from_manual_input(self, tips: list[str]) -> list[str]:
        ...
        find_event_by_id(EventID.ANSWER_REQUESTED).invoke(
            "\n".join(tips), queue)
        try:
            return queue.get(timeout=_MANUAL_ANSWER_TIMEOUT_SECS)
        except Empty:
            warning("等待人工输入答案超时，将继续处理当前题目")
            return []

    def _wait_for_enabled_action(self, action: Locator) -> bool:
    ...
    def _click_action_with_retry(self, selector: str) -> bool:
        ...
        for attempt in range(1, _ACTION_CLICK_RETRIES + 1):
            ...
            if not self._wait_for_enabled_action(action):
                self.last_page.wait_for_timeout(150)
                continue
            try:
                ...
                action.wait_for(state="visible", timeout=500)
                action.click(timeout=_ACTION_CLICK_TIMEOUT_MSECS)
                return True
                ...
                try:
                    ...
                    if self._wait_for_enabled_action(action):
                        action.evaluate("element => element.click()", timeout=1000)
                        return True
                except Exception:
                    pass
                self.last_page.wait_for_timeout(200)
    ...
    def _wait_for_action_or_transition(self, previous_title: str) -> tuple[str, str | None]:
        """Wait for an enabled action or for a React transition already in flight.

        The exam page briefly disables the action button while it replaces the
        current question.  Treating that transient disabled state as a hard
        failure races the page and makes the task stop even though the next
        question is about to appear.
        """
        deadline = monotonic() + _QUESTION_TRANSITION_TIMEOUT_MSECS / 1000
        while monotonic() < deadline:
            if self._is_test_finished():
                return "finished", None
            current_title = self._current_question_title()
            ...
            for selector, action_name in (
                ...
                try:
                    if action.count() > 0 and action.is_visible() and action.is_enabled():
                        return "action", action_name
                except Exception:
                    # React can replace the action node between the checks.
                    pass
            self.last_page.wait_for_timeout(100)
        return "timeout", None

    def _go_to_next_question(self, previous_title: str | None = None) -> bool:
        if previous_title is None:
            previous_title = self._current_question_title()
        had_solution = self._has_solution_visible()
        submitted = False
        final_action = False
        try:
            wait_state, action_name = self._wait_for_action_or_transition(
                previous_title or "")
            ...
            try:
                action_text = clean_string(
                    action.inner_text(timeout=1000)).replace(" ", "")
                final_action = action_name == "next" and "完成" in action_text
            except Exception:
                final_action = False

            if not self._click_action_with_retry(selector):
        ...
        if not self._handle_captcha(captcha_wait):
        ...
        if loading.count() > 0:
            ...
            try:
                loading.wait_for(state="hidden", timeout=_LOADING_TIMEOUT_MSECS)
            except TimeoutError:
                warning("答题加载动画等待超时，将继续检查题目是否已经切换")

        return self._wait_for_question_transition(previous_title or "", had_solution)

    def _current_question_title(self) -> str:
        try:
            ...
            return clean_string(title.inner_text(timeout=2000))
        except Exception:
            return ""

    def _has_solution_visible(self) -> bool:
    ...
    def _wait_for_question_transition(
        ...
        while monotonic() < deadline:
            ...
            if previous_title and current_title and current_title != previous_title:
                return True
            if not had_solution and self._has_solution_visible():
                return True
            self.last_page.wait_for_timeout(100)
        ...
        if self._is_test_finished():
            return True
        final_title = self._current_question_title()
    ...
    def _do_answer(self, elements: Locator, blank: bool, tips: list[str], title: str | None = None) -> bool:
        ...
        def do_answer(answers: list[str]) -> bool:
            ...
            if blank:
                ...
                for i, answer in enumerate(answers):
                    ...
                    answer_handled = self._fill_blank(elements.nth(i), answer)
            ...
            choice_texts = [
                _normalize_choice_text(elements.nth(i).inner_text())
            ...
            ordering = self._is_ordering_choice(elements)
            handled = True
            for index in matched_indices:
                choice = (
                    self._find_ordering_choice(choice_texts[index])
                ...
                handled = self._chose_answer(choice) and handled
            return handled
        ...
        answers_from_ai = get_answer_from_ai(title, tips, blank)
        if do_answer(answers_from_ai):
    ...
    def _find_visible_captcha(self, wait_timeout_msecs: int) -> Locator | None:
    ...
    def _wait_captcha_hidden(
        self, captcha: Locator, timeout_msecs: int = _CAPTCHA_VERIFY_TIMEOUT_MSECS,
    ) -> bool:
        try:
            captcha.wait_for(state="hidden", timeout=timeout_msecs)
            return True
        except TimeoutError:
            try:
    ...
    def _handle_captcha(self, wait_timeout_msecs: int = 300) -> bool:
        try:
            captcha = self._find_visible_captcha(wait_timeout_msecs)
        ...
        try:
            ...
            if config.captcha_local_enabled and config.captcha_local_url.strip():
                try:
                    image = captcha.screenshot(type="png")
                    captcha_box = captcha.bounding_box()
                    ...
                    distance, confidence = request_local_slider_distance(
                    ...
                    if distance is not None:
                        ...
                        if self._drag_captcha_by_distance(
            ...
            for attempt in range(3):
                # The dialog can animate or rerender after a failed attempt.
                slider = captcha.locator(TestSelectors.TEST_CAPTCHA_SLIDER).first
                target = captcha.locator(TestSelectors.TEST_CAPTCHA_TARGET).first
                slider_box = slider.bounding_box()
                target_box = target.bounding_box()
                if slider_box is None or target_box is None:
                    if self._wait_captcha_hidden(captcha, 500):
                        return True
                    warning("第 %d 次拖动前未能刷新滑块坐标" % (attempt + 1))
                    continue
                distance = calculate_slider_max_distance(slider_box, target_box)
                if distance > 0 and self._drag_captcha_by_distance(
                        captcha, slider_box, target_box, distance):
                    return True
                warning("第 %d 次内置滑块拖动未通过" % (attempt + 1))
                self.last_page.wait_for_timeout(500)
            return self._wait_captcha_hidden(captcha, 500)
        except Exception as captcha_error:
            warning("滑块验证码处理失败：%s" % captcha_error)
            return False

    def _drag_captcha_by_distance(
        ...
        self.last_page.mouse.move(start_x, start_y)
        sleep(0.18)
        self.last_page.mouse.down()
        try:
            sleep(0.22)
            for index, (x, y) in enumerate(build_drag_positions(
                    start_x, start_y, distance, steps=58), start=1):
                self.last_page.mouse.move(x, y, steps=1)
                sleep(0.014 if index < 45 else 0.022)
            sleep(0.57)
        finally:
            self.last_page.mouse.up()
        return self._wait_captcha_hidden(captcha)
        ...
        for i in range(modal.count()):
            try:
                if modal.nth(i).is_visible():
                    modal_text = clean_string(modal.nth(i).inner_text(timeout=2000))
    ...
    def _is_ordering_choice(self, elements: Locator) -> bool:
        """Return whether the answer controls are click-to-order modules."""
        for i in range(elements.count()):
            try:
                class_name = elements.nth(i).get_attribute("class", timeout=1000) or ""
    ...
    def _find_ordering_choice(self, target_text: str) -> Locator | None:
        ...
        for i in range(candidates.count()):
            ...
            try:
                class_name = candidate.get_attribute("class", timeout=1000) or ""
                if "fill-answer-click" in class_name:
                    continue
                choice_text = _normalize_choice_text(candidate.inner_text(timeout=1000))
    ...
    def _fill_blank(self, blank: Locator, text: str) -> bool:
        try:
            sleep(uniform(ANSWER_SLEEP_MIN_SECS, ANSWER_SLEEP_MAX_SECS))
            try:
                blank.fill(text, timeout=WAIT_CHOICE_SECS * 1000)
            except Exception:
                blank.evaluate(
                    """
                    (element, value) => {
    ...
    def _chose_answer(self, choice: Locator) -> bool:
        ...
        for attempt in range(1, _CHOICE_CLICK_RETRIES + 1):
            try:
                ...
                if attempt == 1:
                    self.last_page.wait_for_timeout(int(uniform(
                ...
                choice.click(timeout=_CHOICE_CLICK_TIMEOUT_MSECS)
                return True
            except TimeoutError as e:
...
class FunTestTask(_TestTask):
    ...
    def __enter__(self):
        self._goto(POINTS_PAGE)
        self.last_page.locator(Selectors.LOADING).wait_for(
            state="hidden", timeout=WAIT_CHOICE_SECS * 1000)
        cards = self.last_page.locator(PointsSelectors.POINTS_CARDS)
        try:
            cards.last.wait_for(timeout=WAIT_CHOICE_SECS * 1000)
        except TimeoutError as e:
        ...
        for i in range(cards.count()):
            card = cards.nth(i)
            title = clean_string(card.locator(
            ...
            if title in self.handles:
                ...
                try:
                    with self.last_page.context.expect_page(timeout=3000) as event:
                        if action.count() > 0:
                            action.click()
                        else:
                            card.click()
