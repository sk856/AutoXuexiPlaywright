...
class SqliteAnswerSource(_AddSupportedAnswerSource):
    ...
    def get_answer(self, title: str) -> list[str]:
        ...
        if isinstance(answer_str_encoded, tuple) and len(answer_str_encoded) > 0 and isinstance(answer_str_encoded[0], str):
            answer_str = b64decode(answer_str_encoded[0]).decode()
...
def close_all_answer_sources():
    ...
    """
    for answer_source in list(_answer_sources):
        try:
            answer_source.close()
        except Exception as e:
